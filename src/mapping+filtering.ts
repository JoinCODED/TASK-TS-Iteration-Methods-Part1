...
function filteredAndSquared(numbers: number[]): number[] {
 ...
 return numbers.filter(number => number >= 20).map(number => number * number); // replace empty array with what you see is fit
}
...
function filteredAndTripled(numbers: number[]): number[] {
  ...
  return numbers.filter(number => number % 5 === 0).map(number => number * 3); // replace empty array with what you see is fit
}
