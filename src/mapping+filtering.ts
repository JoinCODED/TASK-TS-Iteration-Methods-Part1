...
function filteredAndSquared(numbers: number[]): number[] {
  ...
  return numbers.filter(number => number >= 20).map(n => n**2) ; // replace empty array with what you see is fit
}
...
function filteredAndTripled(numbers: number[]): number[] {
  ...
  return numbers.filter(number => number % 5 ==0).map(n => n*3) ; // replace empty array with what you see is fit
}
