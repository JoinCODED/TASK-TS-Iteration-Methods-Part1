...
function filteredAndSquared(numbers: number[]): number[] {
  ...
  const filterandsquare: number[] = numbers
    .filter((n) => n >= 20)
    .map((n) => n * n);
  return filterandsquare; // replace empty array with what you see is fit
}
...
function filteredAndTripled(numbers: number[]): number[] {
  // write your code here...
  const filterandtriple: number[] = numbers
    .filter((n) => n % 5 === 0)
    .map((n) => n * 3);
  return filterandtriple; // replace empty array with what you see is fit
}
