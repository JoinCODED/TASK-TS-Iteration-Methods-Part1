...
function squaredNumbers(numbers: number[]): number[] {
  ...
  return numbers.map(number => number * number); // replace empty array with what you see is fit
}
...
function doubledNumbers(numbers: number[]): number[] {
  ...
  return numbers.map(number => number * 2); // replace empty array with what you see is fit
}
