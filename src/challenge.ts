...
function toCelsius(temperatures: number[]): number[] {
  ...
  return temperatures.map((n) => (n - 32) * (5 / 9)); // replace empty array with what you see is fit
}
...
function hottestDays(temperatures: number[], threshold: number): number[] {
  ...
  return temperatures.filter((n) => n > threshold); // replace empty array with what you see is fit
}
